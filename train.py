...
def train_dncnn_model():
    ...
    # 构建模型
    model = dncnn_model(depth=17, filters=128, image_channels=3)  # 增加滤波器数量到128
...
#
# from keras.preprocessing.image import ImageDataGenerator
# from model import dncnn_model, compile_model
#
#
#
# #
...
# #
# #
# # def add_noise(images, noise_factor=0.01):
# #     noisy_images = []
# #     for img in images:
# #         noise = np.random.normal(loc=0, scale=noise_factor ** 0.5, size=img.shape)
# #         noisy_img = np.clip(img + noise, 0, 1)
...
# #
# #     # 构建模型
# #     model = dncnn_model(depth=17, filters=128, image_channels=3)
