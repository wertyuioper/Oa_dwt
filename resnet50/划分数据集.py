...
file_path = 'C:/Users\RT\Desktop\DRCNN\daisy_datasetflowers'
flower_class = [cla for cla in os.listdir(file_path) if ".txt" not in cla]

# 创建 训练集train 文件夹，并由3种类名在其目录下创建3个子目录
mkfile('flower_data_daisy/train')
for cla in flower_class:
    mkfile('data_set3/train/' + cla)

# 创建 验证集val 文件夹，并由3种类名在其目录下创建3个子目录
mkfile('flower_data_daisy/val')
for cla in flower_class:
    mkfile('data_set3/val/' + cla)
...
for cla in flower_class:
    ...
    for index, image in enumerate(images):
        ...
        if image in eval_index:
            ...
            new_path = 'data_set3/val/' + cla
            copy(image_path, new_path)  # 将选中的图像复制到新路径

        # 其余的图像保存在训练集train中
        else:
            image_path = cla_path + image
            new_path = 'data_set3/train/' + cla
